...
amazing-hand-cli vendors its `.claude/skills/` from **guildmaster** — the
...
Every vendored `SKILL.md` carries `type: command`. amazing-hand-cli
...
#   - consumer-identifying prose: `guildmaster` → `amazing-hand-cli` (NOT
...
If a re-sync would lose a amazing-hand-cli adaptation, lift the change
...
#     → `which amazing-hand-cli vendors from guildmaster`
