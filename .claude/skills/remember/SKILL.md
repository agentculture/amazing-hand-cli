...
description: >
  ...
  defaults records to this agent's PERSONAL, PRIVATE scope (`--scope amazing-hand-cli
...
scope:** the default personal scope is **private** (`--scope amazing-hand-cli
...
- `--scope NAME` / `--visibility public|private` — record scope. **The wrapper
  ...
  (here, `amazing-hand-cli`). Private records are served only to a recall in the same
